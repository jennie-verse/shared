...
Folio·Cove의 `reading-session`과 Slate·Grove의 `usage-session`을
additive kind로 허용합니다. 기존 kind와 envelope `v: 1`은 그대로 유지합니다.
...
await journal.enqueue(record, { date: '2026-01-01' });
```
...
`item-activity`와 `block-activity`는 객체가 속한 날짜와 사용자가 실제로
...
**ES module 앱** (`<script type="module">` 을 쓰는 앱 — 예: petal)
